...
export default function Booking(props) {
  ...
  React.useEffect(() => {
    ...
    axios
      .post("http://localhost:8000/api/bookingdata", { user_id })
      .then((res) => {
        setData(
          res.data.filter((val) => {
  ...
  function cancle_booking(bookingid) {
    axios
      .delete("http://localhost:8000/api/canclebooking/" + bookingid)
      .then((res) => {
        setData(
          mybooking.filter((val) => {
  ...
  const rendercar = () => {
    ...
    return mybooking.map((mybooking) => {
      ...
      const price = Math.floor(
        diffHours * mybooking.car.cprice +
          diffminutes * (mybooking.car.cprice / 60)
      );
          ...
          <div className="cars" id={row_id}>
            ...
            <div className="car_details">
              <span>
                ...
                <h3>{mybooking.car.location}</h3>{" "}
              </span>
              <h2>
                {mybooking.car.company} {mybooking.car.cname}
              </h2>
              <h3>
                {mybooking.car.seat} {mybooking.car.cartype}
              </h3>
              <br></br>
              ...
              <h2>
                {mybooking.booking.fromlocation.toUpperCase()} To{" "}
                {mybooking.booking.tolocation.toUpperCase()}
              </h2>
              <h2>Booking(D:H:M) - {mybooking.booking.Hours} </h2>
              <h2>Price : {price}</h2>
              <button
                name="carid"
                value={mybooking.car._id}
                onClick={() => {
                  const confirmBox = window.confirm(
                    "Do you really want to cancle booking of " +
                      mybooking.car.company +
                      " " +
                      mybooking.car.cname +
                      "."
                  );
