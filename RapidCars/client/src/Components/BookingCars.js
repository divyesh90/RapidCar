...
export default function BookingCars() {
  ...
  React.useEffect(() => {
    axios
      .post("http://localhost:8000/api/systembookingdata")
      .then((res) => {
        setbooking(res.data);
        setData(res.data);
      })
  ...
  function BookingComplete(carid) {
    axios
      .put("http://localhost:8000/api/bookingcomplete/" + carid)
      .then((res) => {
  ...
  function rendercar() {
    var num = 0;
    booking.sort((a, b) =>
      new Date(a.booking.todate) < new Date(b.booking.todate) ? 1 : -1
    );
      ...
      if (booking.booking.IsRuning == true) {
        var html = (
          <a
            ...
            title="Delete"
            data-toggle="tooltip"
            onClick={() => BookingComplete(booking.booking._id)}
          >
      ...
      return (
        <>
          <tr>
            ...
            <td>
              <a>
                <img src={path} className="avatar" alt="photo" />{" "}
                {booking.car.cname}
              </a>
            </td>
                    ...
                    <div className="column">
                      <div class="list_button">
                        <div class="button">
                          ...
                          <button
                            id="reserve"
                            onClick={() => booking_history_sort(true)}
                          >
                            Running
                          </button>
                        </div>
                        <div class="button">
                          ...
                          <button
                            id="unreserve"
                            onClick={() => booking_history_sort(false)}
                          >
                            History
                          </button>
                        </div>
                        <div class="button">
                          ...
                          <button
                            id="all"
                            onClick={() => booking_history_sort("all")}
                          >
                            All Car
                          </button>
                        </div>
