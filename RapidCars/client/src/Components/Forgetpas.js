...
export default function Forgetpas() {
  ...
  function ResetPassword() {
    if (validate_input(user) == true) {
      axios
        .post("http://localhost:8000/api/resetpass", user)
        .then((res) => {
          if (res.data.email) {
            history.push({
              pathname: "/login",
            ...
            console.log("Something Went wrong Password not updated");
          }
        })
        .catch((err) => {
  ...
  function SendOtp() {
    ...
    if (!user.email.match(mailformat) || user.email == "") {
      ...
      axios
        .post("http://localhost:8000/api/sendotp", user)
        .then((res) => {
          if (res.data.result) {
  ...
  function ValidateOtp() {
    if (user.otp.length < 6 || user.otp == "") {
      document.getElementById("validate_otp").innerHTML =
        "OTP must be 6 latter";
    } else {
      axios
        .post("http://localhost:8000/api/cmpotp", user)
        .then((res) => {
            ...
            <div className="card">
              ...
              <div className="card-body">
                <form>
                  ...
                  <div id="otp_block">
                    <div className="validation" id="validate_otp">
                      {" "}
                    </div>
                    <div className="input-group form-group">
                      <div className="input-group-prepend">
